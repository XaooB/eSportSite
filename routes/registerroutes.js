const request = require('request-promise'),
  bcrypt = require('bcrypt'), {User} = require('../models/user'), {getDate} = require('./../public/js/getDate');
...
exports.registerPost = (req, res) => {
  bcrypt.hash(req.body.password, 10).then((hashedassword) => {
    ...
    if (response.length === 0 || response === null || response === undefined || response === '') {
      res.render('register', {Error: 'Udownodnij, że nie jesteś BOT-em!'});
      return false;
    }
    ...
    request(verificationUrl).then((data) => {
      ...
      if (data.success !== undefined && !data.success) {
        res.render('register', {Error: 'Błąd podczas walidacji. Spróbuj ponownie!'})
      };

      let newUser = User({username: req.body.username.toLowerCase(), email: req.body.email, password: hashedassword, createdAt: getDate()}).save().then((data) => {

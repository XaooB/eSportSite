const {User} = require('../models/user'), {Comment} = require('../models/comment'), {Article} = require('../models/article'),
...
exports.lostGet = (req, res) => {
  res.render('lostPassword', {title: 'Resetowanie hasła'});
};

exports.loginPost = (req, res) => {
  User.findOne({username: req.body.username.toLowerCase().trim()}).then((user) => {
    return bcrypt.compare(req.body.password, user.password).then((result) => {
      ...
      res.render('login', {Error: 'Hasło nieprawidłowe. Spróbuj ponownie!'})
    });
  }).catch((err) => {
    console.log('MONGOOSE ERROR: ' + err.message);
    res.render('login', {Error: `Niestety, ale użytkownik ${req.body.username} nie istnieje w naszej bazie! Proszę się zajerejstrować <a href='/register'>tutaj</a>!`})
  })
};
...
exports.me = (req, res) => {
  User.findOne({username: req.user.username.toLowerCase()}).then((user) => {
    ...
    console.log(err);
    res.json({"Error": "Wystąpił błąd podczas wyszukiwania użytkownika w bazie. Sprawdź konsole."})
  });
}
...
exports.profil = (req, res) => {
  User.findOne({username: req.params.username.toLowerCase()}).then((user) => {
    ...
    console.log(err);
    res.json({"Error": "Takiego użytkownika nie ma w naszej bazie"})
  });
}
